...
(function ($) {
        ...
        $.each(this.header.fields, function(j, field) {
            ...
            if (column.checkbox || column.radio) {
                ...
                text = [sprintf(that.options.cardView ?
                        '<div class="card-views %s">' : '<td class="bs-checkbox %s">', column['class'] || ''),
                    '<input' +
                    sprintf(' data-index="%s"', i) +
                ...
                text = that.options.cardView ? ['<div class="card-views">',
                ...
                // Hide empty data on Card views when smartDisplay is set to true.
                if (that.options.cardView && that.options.smartDisplay && value === '') {
                    // Should set a placeholder for event binding correct fieldIndex
                    text = '<div class="card-views"></div>';
                }
            }
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        this.$body.find('> tr[data-index] > td > .detail-icon').off('click').on('click', function () {
            var $this = $(this),
                ...
                index = $tr.data('index'),
                row = data[index]; // Fix #980 Detail views, when searching, returns wrong row

            // remove and update
            if ($tr.next().is('tr.detail-views')) {
                $this.find('i').attr('class', sprintf('%s %s', that.options.iconsPrefix, that.options.icons.detailOpen));
                that.trigger('collapse-row', index, row);
                $tr.next().remove();
            } else {
                $this.find('i').attr('class', sprintf('%s %s', that.options.iconsPrefix, that.options.icons.detailClose));
                $tr.after(sprintf('<tr class="detail-views"><td colspan="%s"></td></tr>', $tr.find('td').length));
        ...
        $.each(this.header.events, function (i, events) {
            ...
            for (var key in events) {
                that.$body.find('>tr:not(.no-records-found)').each(function () {
                    var $tr = $(this),
                        $td = $tr.find(that.options.cardView ? '.card-views' : 'td').eq(fieldIndex),
                        index = key.indexOf(' '),
    ...
    BootstrapTable.prototype.expandRow_ = function (expand, index) {
        ...
        if ($tr.next().is('tr.detail-views') === (expand ? false : true)) {
    ...
    BootstrapTable.prototype.expandAllRows = function (isSubTable) {
        if (isSubTable) {
            ...
            } else if (!$tr.next().next().is('tr.detail-views')) {
            ...
            if (executeInterval) {
                try {
                    idInterval = setInterval(function () {
                        detailIcon = that.$body.find("tr.detail-views").last().find(".detail-icon");
